...
void L3_369engine_setPlayerOrder(const char playerOrderIn[L3_MAX_PLAYERS][L3_MAX_NICKNAME_LEN], uint8_t count)
{
    if (count > L3_MAX_PLAYERS) {
        count = L3_MAX_PLAYERS;
    }
    playerCount = count;
...
static uint8_t count369Digits(uint32_t number)
{
    ...
    while (number > 0) {
        ...
        if (digit == 3 || digit == 6 || digit == 9) {  // Fix: ; -> ||
            count++;
        }
        number /= 10;
    }
...
{
    if (outValue == NULL || outValueLen == 0) {  // Fix: ; -> ||
        return NULL;
    }
